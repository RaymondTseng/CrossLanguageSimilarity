...
class CNN:
    ...
    def build_network(self):
        ...
        with tf.name_scope('output'):
            ...
            # new
            # self.logits = tf.nn.softmax(self.logits, dim=1)
            # class_scores = tf.range(self.class_num, dtype=tf.float32)
            # class_scores = tf.reshape(class_scores, [-1, 1])
            # self.logits = tf.matmul(self.logits, class_scores)
