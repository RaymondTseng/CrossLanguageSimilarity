...
tf.flags.DEFINE_integer('attention_length', 6, 'the number of attention window (default 6)')
tf.flags.DEFINE_integer('attention_num', 16, 'the number of attention units (default 32)')


# Data Parameters
tf.flags.DEFINE_string('train_path', '/home/raymond/Downloads/es-en/cls-train.txt', 'train data')
tf.flags.DEFINE_string('dev_path', '/home/raymond/Downloads/es-en/cls-dev.txt', 'dev data')
tf.flags.DEFINE_string('test_path', '/home/raymond/Downloads/es-en/cls-test.txt', 'test data')
...
test_sources = utils.word2id(test_sources, source_word2idx, FLAGS.seq_length)
test_targets = utils.word2id(test_targets, target_word2idx, FLAGS.seq_length)
...
with tf.Graph().as_default():
    ...
    with session.as_default():
        ...
        # model = CNNAttention(FLAGS.seq_length, FLAGS.class_num, map(int, FLAGS.filter_sizes.split(',')), FLAGS.filters_num,
        #                FLAGS.embedding_size, FLAGS.attention_num, FLAGS.learning_rate, FLAGS.l2_reg_lambda)

        # model = ConvLSTM(FLAGS.seq_length, FLAGS.hidden_size, FLAGS.layer_num, FLAGS.class_num,
        #                FLAGS.learning_rate, FLAGS.l2_reg_lambda)

        # model = BiLSTMAttention(FLAGS.seq_length, FLAGS.hidden_size, FLAGS.layer_num, FLAGS.attention_length, FLAGS.class_num,
        #                FLAGS.learning_rate, FLAGS.l2_reg_lambda)

        model = ConvLSTM(FLAGS.seq_length, FLAGS.hidden_size, FLAGS.layer_num, 3,
                        FLAGS.embedding_size, FLAGS.class_num, FLAGS.learning_rate, FLAGS.l2_reg_lambda)
        ...
        for step in range(FLAGS.epochs_num):
            ...
            summary, _, pearson, loss, accuracy = session.run([merged] + ops, feed_dict=feed_dict)
            train_writer.add_summary(summary, global_step=step + 1)

            print '--- training step %s --- loss: %.3f --- pearson: %.3f --- accuracy: %.3f ---' % (step + 1, loss, pearson, accuracy)

            if (step + 1) % FLAGS.evaluate_every == 0:
                ops, feed_dict = model.dev_step(dev_sources, dev_targets, dev_scores)
                summary, pearson, loss, accuracy = session.run([merged] + ops, feed_dict=feed_dict)
                dev_writer.add_summary(summary, global_step=step + 1)
                print '--- evaluate step %s --- loss: %.3f --- pearson: %.3f --- accuracy: %.3f ---' % (step + 1, loss, pearson, accuracy)

        ops, feed_dict = model.dev_step(test_sources, test_targets, test_scores)
        pearson, loss, accuracy = session.run(ops, feed_dict=feed_dict)
        print '--- test step --- loss: %.3f --- pearson: %.3f --- accuracy: %.3f ---' % (loss, pearson, accuracy)
